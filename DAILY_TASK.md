...
這條有自動檢查：`build_site.py` 會把當天超標的段落印在 stderr，指出是哪一篇、
哪一節、哪個語言、幾個字。**它只警告不擋 build**（段落多長是判斷題，不該因為文風
擋住當天的產出），而且只看最新一天——警告整個資料庫會印出一百多行，那種長度的
警告沒有人會讀。看到警告就回去把那幾段拆開，不要視而不見。
